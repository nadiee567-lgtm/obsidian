...
def test_port_sensitive():
    ...
    assert sum(1 for x in h if x.rule == 'sensitive-port') == 1
...
def test_cert_expired():
    ...
    assert reglas.count('cert-expired') == 1
...
def test_ip_malicious_is_critical():
    ...
    ip.tag('malicious')
    h = correlate(store)
    assert h[0].rule == 'ip-malicious' and h[0].severity == 'critical'
...
def test_order_per_severity():
    store = Store()
    store.create('email', 'x@y.com').tag('spoofable')
    store.create('ip', '45.0.0.1').tag('malicious')
    store.create('port', '1.1.1.1:3306')
