...
def _store_demo():
    ...
    ip = store.create('ip', '93.184.216.34', properties={'country': 'US'})
...
def test_report_has_sections():
    ...
    html = generate_report(store, findings=h, score=20,
...
def test_report_no_findings():
    store, _, _ = _store_demo()
    html = generate_report(store, findings=[], score=0)
